"""
Run Final Unseen Holdout v4 — ONCE.

This wrapper exists so the one-shot rule is enforced by code rather than by
memory, and so an infrastructure failure is never mistaken for a v4 result.

Consumption rule (what “uses up” the unseen pack):
  - Preflight / pre-scoring faults do NOT consume v4. They write only
    reports/V4_PRECHECK_FAILURE.json and may be fixed and retried.
  - The moment the first scored clip finishes, V4_RUN_RECORD.json is created
  ...
  - Crash / abort after STARTED → VOID (partial scores already observed).
    ...
    not implemented here; VOID means a new unseen pack (v5) is required.
...
Preflight (all must hold, or nothing runs; no consuming record):
  ...
  4. reports/FINAL_LATENCY_CHECK.json says LATENCY_READY_FOR_V3 or
     LATENCY_READY_FOR_V4 and both cohorts are warm-valid
  5. no consuming V4_RUN_RECORD (STARTED / VALID / VOID / legacy in-progress)

Outcome classification:
  PRECHECK_FAILED — infrastructure fault before first scored clip (lock,
                    missing WAV, cold latency gate, crash before scoring).
                    Fix and run again. Not a v4 result.
  VALID           — first scored clip started and the suite finished. Whatever
                    it scored is the v4 result, including Whisper variance.
  VOID            — scoring started then aborted / incomplete / post-score
    ...
    python scripts/run_holdout_v4.py
    python scripts/run_holdout_v4.py --preflight-only
"""
...
RUN_RECORD = REPORTS / "V4_RUN_RECORD.json"
PRECHECK_FAILURE = REPORTS / "V4_PRECHECK_FAILURE.json"
LATENCY_OK_VERDICTS = frozenset({"LATENCY_READY_FOR_V3", "LATENCY_READY_FOR_V4"})
...
def write_precheck_failure(problems: list[str], info: dict) -> None:
    _write_json(
        PRECHECK_FAILURE,
        {
            ...
            "info": info,
            "note": (
                "Precheck only — V4_RUN_RECORD was not created. "
                "Fix and run again; v4 is not consumed."
            ),
        },
    )
...
def preflight() -> tuple[bool, list[str], dict]:
    ...
    from scripts.lock_holdout_v4 import verify as verify_lock  # type: ignore
    ...
    if not gate_path.is_file():
        ...
        if gate.get("verdict") not in LATENCY_OK_VERDICTS:
            problems.append(
                f"latency gate verdict is {gate.get('verdict')} "
                f"(need one of {sorted(LATENCY_OK_VERDICTS)})"
            )
    ...
    if RUN_RECORD.is_file():
        ...
        if _record_is_consuming(prev):
            problems.append(
                f"v4 already consumed at {prev.get('started_at') or prev.get('first_scored_at')} "
...
def write_verdict(outcome: str, detail: dict, problems: list[str]) -> None:
    ...
    (REPORTS / "FINAL_UNSEEN_HOLDOUT_V4_VERDICT.json").write_text(
    ...
    if outcome == "PRECHECK_FAILED":
        lines += [
            "This is **not** a v4 result and **does not consume** the pack.",
            "Only `V4_PRECHECK_FAILURE.json` was written. Fix and run again.",
            "",
        ...
        lines += [
            ...
            "A new unseen pack (v5) is required if you need another official gate.",
            "",
        ] + [f"- {p}" for p in problems] + [""]
        ...
        lines += [
            ...
            (
                ...
                else "- A gate failed. If you fix a real bug after seeing this, v4 is VOID "
                     "and a new unseen v5 pack is required."
            ),
            "",
        ]
    (REPORTS / "FINAL_UNSEEN_HOLDOUT_V4_VERDICT.md").write_text("\n".join(lines), encoding="utf-8")
...
def main() -> int:
    ...
    def on_scored_clip(scored_i: int, result: Any) -> None:
        ...
        print(
            f"V4_RUN_RECORD STARTED at first scored clip "
    ...
    report_path = Path(payload.get("report_json") or (REPORTS / "FINAL_UNSEEN_HOLDOUT_V4_REPORT.json"))
    ...
    print(f"\nv4 VALID — gates pass: {detail['all_gates_pass']}")
    print("read reports/FINAL_UNSEEN_HOLDOUT_V4_VERDICT.md")
